...
namespace jni::avdev
{
    ...
    {
        return createDefaultAudioDevice(eCapture);
    }

    AudioDevicePtr WindowsAudioDeviceManager::getDefaultAudioPlaybackDevice()
    {
        return createDefaultAudioDevice(eRender);
    }
    ...
    void WindowsAudioDeviceManager::enumerateDevices(EDataFlow dataFlow)
    {
        ...
        if (!taskQueueFactory)
        {
            throw Exception("Create TaskQueueFactory failed");
        }
        ...
        if (!audioModule)
        {
            throw Exception("Create AudioDeviceModule failed");
        }
        if (audioModule->Init() != 0)
        {
            throw Exception("Initialize AudioDeviceModule failed");
        }

        bool enumCaptureDevices = false;
        bool enumRenderDevices = false;

        if (dataFlow == eAll || dataFlow == eCapture)
        {
            enumCaptureDevices = true;
        }
        if (dataFlow == eAll || dataFlow == eRender)
        {
        ...
        if (enumCaptureDevices)
        {
            ...
            for (int i = 0; i < deviceCount; ++i)
            {
                ...
                {
                    auto device = std::make_shared<AudioDevice>(name, guid);

                    insertAudioDevice(device, eCapture);
                }
            }
        }
        if (enumRenderDevices)
        {
            ...
            for (int i = 0; i < deviceCount; ++i)
            {
                if (audioModule->PlayoutDeviceName(i, name, guid) == 0)
                {
                    ...
                    insertAudioDevice(device, eRender);
                }
            }
        }
    }
    ...
    AudioDevicePtr WindowsAudioDeviceManager::createDefaultAudioDevice(const EDataFlow& dataFlow)
    {
        ...
        auto device = std::make_shared<AudioDevice>(name, id);
