...
namespace jni::ProcessingConfig
{
    webrtc::ProcessingConfig toNative(JNIEnv* env, const JavaRef<jobject>& javaType)
    {
        ...
        {
            auto key = JavaEnums::toNative<webrtc::ProcessingConfig::StreamName>(env, entry.first);
    ...
    JavaLocalRef<jobject> toJava(JNIEnv* env, webrtc::ProcessingConfig config)
    {
        ...
        jobject obj = env->NewObject(javaClass->cls, javaClass->ctor,
                                     static_cast<JavaLocalRef<jobject>>(streams).get());
        ExceptionCheck(env);
