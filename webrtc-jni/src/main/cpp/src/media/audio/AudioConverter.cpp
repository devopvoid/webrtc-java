...
namespace jni
{
    ...
    public:
            ...
            : AudioConverter(srcFrames, srcChannels, dstFrames, dstChannels)
        {
            ...
            resampler->InitializeIfNeeded(static_cast<int>(srcFrames * 100), static_cast<int>(dstFrames * 100),
                                          srcChannels);
        }
    ...
    std::unique_ptr<AudioConverter> AudioConverter::create(size_t srcFrames, size_t srcChannels, size_t dstFrames,
                                                           size_t dstChannels)
    {
        ...
        if (srcChannels > dstChannels)
        {
            if (srcFrames != dstFrames)
            {
                ...
                converters.push_back(
                    std::make_unique<ChannelConverter>(srcFrames, srcChannels, srcFrames, dstChannels));
                converters.push_back(
                    std::make_unique<ResampleConverter>(srcFrames, dstChannels, dstFrames, dstChannels));
        ...
        else if (srcChannels < dstChannels)
        {
            if (srcFrames != dstFrames)
            {
                std::vector<std::unique_ptr<AudioConverter>> converters;
                converters.push_back(
                    std::make_unique<ResampleConverter>(srcFrames, srcChannels, dstFrames, srcChannels));
                converters.push_back(
                    std::make_unique<ChannelConverter>(dstFrames, srcChannels, dstFrames, dstChannels));
