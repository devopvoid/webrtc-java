...
namespace jni::AudioProcessingConfig
{
    webrtc::AudioProcessing::Config toNative(JNIEnv* env, const JavaRef<jobject>& javaType)
    {
        ...
        config.capture_level_adjustment.enabled = captureLevelAdjustment.getBoolean(
            javaCaptureLevelAdjustmentClass->enabled);
        config.capture_level_adjustment.pre_gain_factor = captureLevelAdjustment.getFloat(
            javaCaptureLevelAdjustmentClass->preGainFactor);
        config.capture_level_adjustment.post_gain_factor = captureLevelAdjustment.getFloat(
            javaCaptureLevelAdjustmentClass->postGainFactor);
        config.capture_level_adjustment.analog_mic_gain_emulation = toAnalogMicGainEmulation(
            env, captureLevelAdjustment.getObject(javaCaptureLevelAdjustmentClass->analogMicGainEmulation));

        config.echo_canceller.enabled = echoCanceller.getBoolean(javaEchoCancellerClass->enabled);
        config.echo_canceller.enforce_high_pass_filtering = echoCanceller.getBoolean(
            javaEchoCancellerClass->enforceHighPassFiltering);
        ...
        if (nsLevel.get())
        {
            config.noise_suppression.level = JavaEnums::toNative<
                webrtc::AudioProcessing::Config::NoiseSuppression::Level>(env, nsLevel);
        }
    ...
    webrtc::AudioProcessing::Config::CaptureLevelAdjustment::AnalogMicGainEmulation toAnalogMicGainEmulation(
        JNIEnv* env, const JavaLocalRef<jobject>& javaType)
    {
        ...
        analog_mic_gain_emulation.initial_level = analogMicGainEmulation.getInt(
            javaAnalogMicGainEmulationClass->initialLevel);

        return analog_mic_gain_emulation;
    }


    webrtc::AudioProcessing::Config::GainController2 toGainController2(JNIEnv* env,
                                                                       const JavaLocalRef<jobject>& javaType)
    {
        ...
        gainController.fixed_digital.gain_db = gainControlFixedDigital.getFloat(
            javaGainControlFixedDigitalClass->gainDb);
        gainController.adaptive_digital.enabled = gainControlAdaptiveDigital.getBoolean(
            javaGainControlAdaptiveDigitalClass->enabled);
        gainController.adaptive_digital.dry_run = gainControlAdaptiveDigital.getBoolean(
            javaGainControlAdaptiveDigitalClass->dryRun);
        gainController.adaptive_digital.vad_reset_period_ms = gainControlAdaptiveDigital.getInt(
            javaGainControlAdaptiveDigitalClass->vadResetPeriodMs);
        gainController.adaptive_digital.adjacent_speech_frames_threshold = gainControlAdaptiveDigital.getInt(
            javaGainControlAdaptiveDigitalClass->adjacentSpeechFramesThreshold);
        gainController.adaptive_digital.max_gain_change_db_per_second = gainControlAdaptiveDigital.getFloat(
            javaGainControlAdaptiveDigitalClass->maxGainChangeDbPerSecond);
        gainController.adaptive_digital.max_output_noise_level_dbfs = gainControlAdaptiveDigital.getFloat(
            javaGainControlAdaptiveDigitalClass->maxOutputNoiseLevelDbfs);
    ...
    JavaAudioProcessingConfigClass::JavaAudioProcessingConfigClass(JNIEnv* env)
    {
        ...
        captureLevelAdjustment = GetFieldID(env, cls, "captureLevelAdjustment",
                                            "L" PKG_AUDIO "AudioProcessingConfig$CaptureLevelAdjustment;");
        ...
        noiseSuppression = GetFieldID(env, cls, "noiseSuppression",
                                      "L" PKG_AUDIO "AudioProcessingConfig$NoiseSuppression;");
        residualEchoDetector = GetFieldID(env, cls, "residualEchoDetector",
                                          "L" PKG_AUDIO "AudioProcessingConfig$ResidualEchoDetector;");
        transientSuppression = GetFieldID(env, cls, "transientSuppression",
                                          "L" PKG_AUDIO "AudioProcessingConfig$TransientSuppression;");
    ...
    JavaCaptureLevelAdjustmentClass::JavaCaptureLevelAdjustmentClass(JNIEnv* env)
    {
        ...
        analogMicGainEmulation = GetFieldID(env, cls, "analogMicGainEmulation",
                                            "L" PKG_AUDIO
                                            "AudioProcessingConfig$CaptureLevelAdjustment$AnalogMicGainEmulation;");
    }
    ...
    JavaGainControlClass::JavaGainControlClass(JNIEnv* env)
    {
        ...
        enabled = GetFieldID(env, cls, "enabled", "Z");
        fixedDigital = GetFieldID(env, cls, "fixedDigital",
                                  "L" PKG_AUDIO "AudioProcessingConfig$GainControl$FixedDigital;");
        adaptiveDigital = GetFieldID(env, cls, "adaptiveDigital",
                                     "L" PKG_AUDIO "AudioProcessingConfig$GainControl$AdaptiveDigital;");
    }
