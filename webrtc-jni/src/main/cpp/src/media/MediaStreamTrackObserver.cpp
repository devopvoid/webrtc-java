...
namespace jni
{
    MediaStreamTrackObserver::MediaStreamTrackObserver(JNIEnv* env, const JavaGlobalRef<jobject>& javaTrack,
    ...
    void MediaStreamTrackObserver::OnChanged()
    {
        ...
        if (eventType == MediaStreamTrackEvent::mute)
        {
            if (track->enabled() != trackEnabled)
            {
                ...
                env->CallVoidMethod(javaTrack, javaClass->onTrackMute, createJavaTrack(env).release(),
                                    !track->enabled());
                ExceptionCheck(env);
            }
        }
    ...
    {
        if (auto t = dynamic_cast<const webrtc::AudioTrackInterface*>(track))
        {
            return JavaFactories::create(env, t);
        }
        if (auto t = dynamic_cast<const webrtc::VideoTrackInterface*>(track))
        {
            return JavaFactories::create(env, t);
        }
