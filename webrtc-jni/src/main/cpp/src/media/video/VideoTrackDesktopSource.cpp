...
namespace jni
{
    ...
    void VideoTrackDesktopSource::OnCaptureResult(webrtc::DesktopCapturer::Result result,
    ...
    void VideoTrackDesktopSource::process(std::unique_ptr<webrtc::DesktopFrame>& frame)
    {
        ...
        if (conversionResult >= 0)
        {
            if (!maxFrameSize.is_empty())
            {
                ...
                {
                    double scale = max_width / static_cast<double>(adapted_width);
                    adapted_width = max_width;
                    adapted_height = static_cast<int>(adapted_height * scale);
                }
                else if (adapted_height > max_height)
                {
                    double scale = max_height / static_cast<double>(adapted_height);
                    adapted_width = static_cast<int>(adapted_width * scale);
