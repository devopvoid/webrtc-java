...
namespace jni::RTCRtpParameters
{
    JavaLocalRef<jobject> toJava(JNIEnv* env, const webrtc::RtpParameters& parameters)
    {
        ...
        env->SetObjectField(object, javaClass->headerExtensions,
                            JavaList::toArrayList(env, parameters.header_extensions, &RTCRtpHeaderExtension::toJava));
        env->SetObjectField(object, javaClass->codecs,
                            JavaList::toArrayList(env, parameters.codecs, &RTCRtpCodecParameters::toJava));
