...
namespace jni
{
    ...
    void PeerConnectionObserver::OnIceCandidateError(const std::string& address, int port, const std::string& url,
                                                     int error_code, const std::string& error_text)
    {
        JNIEnv* env = AttachCurrentThread();

        JavaLocalRef<jobject> event = RTCPeerConnectionIceErrorEvent::toJava(
            env, address, port, url, error_code, error_text);
    ...
    void PeerConnectionObserver::OnIceCandidatesRemoved(const std::vector<cricket::Candidate>& candidates)
    {
        ...
        const auto eventClass = JavaClasses::get<
            RTCPeerConnectionIceErrorEvent::JavaRTCPeerConnectionIceErrorEventClass>(env);

        try
        {
            JavaLocalRef<jobjectArray> jCandidates = JavaArray::createObjectArray(
                env, candidates, eventClass->cls, &RTCIceCandidate::toJavaCricket);

            env->CallVoidMethod(observer, javaClass->onIceCandidatesRemoved, jCandidates.get());
        }
        catch (const Exception& e)
        {
            env->Throw(JavaRuntimeException(env, e.what()));
        }
        catch (...)
        {
            ThrowCxxJavaException(env);
