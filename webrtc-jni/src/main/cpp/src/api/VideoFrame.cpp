...
namespace jni
{
    ...
    namespace I420Buffer
    {
        JavaLocalRef<jobject> toJava(JNIEnv* env, const rtc::scoped_refptr<webrtc::I420BufferInterface>& buffer)
        {
            ...
            jobject yBuffer = env->NewDirectByteBuffer(const_cast<uint8_t*>(buffer->DataY()),
                                                       static_cast<jlong>(buffer->StrideY()) * buffer->height());
            jobject uBuffer = env->NewDirectByteBuffer(const_cast<uint8_t*>(buffer->DataU()),
                                                       static_cast<jlong>(buffer->StrideU()) * buffer->ChromaHeight());
            jobject vBuffer = env->NewDirectByteBuffer(const_cast<uint8_t*>(buffer->DataV()),
                                                       static_cast<jlong>(buffer->StrideV()) * buffer->ChromaHeight());

            jobject jBuffer = env->NewObject(javaClass->cls, javaClass->ctor, buffer->width(), buffer->height(),
                                             yBuffer, buffer->StrideY(), uBuffer, buffer->StrideU(), vBuffer,
                                             buffer->StrideV());
