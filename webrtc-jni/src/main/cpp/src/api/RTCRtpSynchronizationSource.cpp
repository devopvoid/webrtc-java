...
namespace jni::RTCRtpSynchronizationSource
{
    JavaLocalRef<jobject> toJava(JNIEnv* env, const webrtc::RtpSource& source)
    {
        ...
        jlong timestamp = source.timestamp_ms();
        jlong sourceId = source.source_id();
        jdouble audioLevel = source.audio_level().value_or(0);
        jlong rtpTimestamp = source.rtp_timestamp();
    ...
    webrtc::RtpSource toNative(JNIEnv* env, const JavaRef<jobject>& source)
    {
        ...
        return webrtc::RtpSource(
            obj.getLong(parentClass->timestamp),
