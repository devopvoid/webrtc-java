...
(JNIEnv* env, jobject caller, jlong sourceId, jboolean isWindow)
{
    ...
    videoSource->setSourceId(sourceId, static_cast<bool>(isWindow));
}
...
(JNIEnv* env, jobject caller)
{
    rtc::scoped_refptr<jni::VideoTrackDesktopSource> videoSource = new rtc::RefCountedObject<
        jni::VideoTrackDesktopSource>();
