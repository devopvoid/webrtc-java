 ...
 jint scaleW, jint scaleH)
{
    ...
    auto dst_y = static_cast<uint8_t*>(env->GetDirectBufferAddress(jDstY));
    auto dst_u = static_cast<uint8_t*>(env->GetDirectBufferAddress(jDstU));
    auto dst_v = static_cast<uint8_t*>(env->GetDirectBufferAddress(jDstV));
